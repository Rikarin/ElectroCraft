...
class NanoMinerItem : ModToolItem(ModToolMaterials.NANO, FabricItemSettings().fireproof()) {
    ...
    fun getAttackDamage() = material.attackDamage

    override fun isSuitableFor(state: BlockState): Boolean {
        return state.isIn(BlockTags.PICKAXE_MINEABLE) || state.isIn(BlockTags.AXE_MINEABLE) || state.isIn(BlockTags.HOE_MINEABLE) || state.isIn(BlockTags.SHOVEL_MINEABLE)
    }
}
